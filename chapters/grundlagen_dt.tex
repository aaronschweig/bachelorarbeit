...
\begin{itemize}
    \item Simulationen (wurde von der NASA und der AirForce verwendet und erfolgreich eingesetzt)
    \item Medizinische Anwendung
    \item Industry 4.0 \autocite[S. 2409]{tao2018digital}
    \begin{itemize}
        \item Nützlich zum monitoren von dem Produktionsprozess, da DTs die Möglichkeit haben, in real time Updates zu liefern
        \item They argued that, since DTs can synchronize the physical and virtual spaces, human operators can depend on DTs to monitor a complex production process, make timely adjustments, and optimize the process \autocites{tao2018digital}{weyer2016future}
        \item DTs can facilitate the adjustment of production operations based on both practical situation and simulation. Rosen et al. discussed the application of DTs in production operations. Since DTs could integrate a variety of data (e.g., environment data, operational data, and process data), autonomous systems can respond to state changes even during an ongoing operation [27].\autocite{rosen2015importance}
        \item By connecting computer simulation with the physical system, the DT could reduce material waste and prolong machine lifetime \autocite{vachalek2017digital}
    \end{itemize}
    \item Prognostics-Health Management
    \begin{itemize}
        \item Tuegel et al. applied the DT to predict the structural life of the aircraft through multiphysics modeling, multiscale damage modeling, integration of the structural ﬁnite-element model (FEM) and damage models, uncertainty quantiﬁcation, and high-resolution structural analysis. They reported that the DT could facilitate the management of aircraft service life [13].\autocite{tuegel2011reengineering}
    \end{itemize}
    \item Beretis angewendet von: \autocite{tao2018digital}
    \begin{itemize}
        \item Siemens
        \item Airbus
        \item SAP SE
