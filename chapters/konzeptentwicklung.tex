...
\begin{example}[Request-Response innerhalb von MQTT] Eine examplarische Methode zur Integration eines Request-Response Pattern kann folgender Abbildung entnommen werden. Dabei ist zu erkennen, dass das Zeil mithilfe einer bestimmten Topicstruktur erreicht wird. Diese Topicstrukturen können innerhalb von MQTT nicht erzwungen werden, sodass nicht sichergestellt ist, dass dieses Konzept zuverlässig funktionert.
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}[node distance=2cm]
            ...
            \draw [arrow, thick] (dev2) -- node[anchor=south] {subscribe response/<unique identifier>} (dr2);
        \end{tikzpicture}
...
\section{Nutzung verschiedener Kommunikationsprotokolle}
...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance=5cm]
        \node (device) [io] {Endgerät};
        \node (hono) [process, right of=device] {Eclipse Hono};
        \node (ditto) [process, below of=hono] {Eclipse Ditto};
        \node (vorto) [process, right of=ditto, fill=blue!50] {Eclipse Vorto};
        \node (application) [startstop, below of=device] {External Application};
        \node (connector) [process, fill=green!30, right of=hono] {Connector};
    
        \draw [arrow] (device) -- (hono);
        \draw [arrow] (hono) -- (ditto);
        \draw [arrow] (ditto) -- (application);
        \draw [arrow] (hono) -- (connector);
        \draw [arrow] (connector) -- (ditto);
        \draw [arrow] (vorto) -- (ditto);
    \end{tikzpicture}
